...
- paint the shared graph with MIHI1's styled lowering, and — a later rung — light the read lane's origin in its own design-system token, so a reader sees at a glance which lanes came from the web and which from their own hand;
