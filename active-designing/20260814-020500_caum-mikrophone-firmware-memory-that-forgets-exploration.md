# CAUM0 — the memory that forgets: a session buffer that holds only while powered

**Stamp:** `20260814.020500` · **Language:** EN · **Voice:** Kyri · **Style:** Radiant
**Status:** Self-approved design round (Season A · waymark **CAUM** · Mikrophone firmware journey · founding rung CAUM0)
...
- `carry(session)` — read back the committed payload as bytes to hand to Comlink (a later CAUM rung wires the actual wire). Uncommitted, it returns nothing — there is nothing to carry.
...
*A device that forgets on purpose owes its keeper a proof that it truly forgets. CAUM0 writes that proof first, so every surface built on it inherits a session that leaks nothing it was never told to keep.*
