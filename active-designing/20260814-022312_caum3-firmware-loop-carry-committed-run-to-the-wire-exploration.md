# CAUM3 — the firmware loop: a committed run carries out to the wire

**Stamp:** `20260814.022312` · **Language:** EN · **Voice:** Kyri · **Style:** Radiant
**Status:** Self-approved design round (Season A · waymark **CAUM** · Mikrophone firmware journey · rung CAUM3)
**Kin:** [`20260814-020500_caum-mikrophone-firmware-memory-that-forgets-exploration.md`](20260814-020500_caum-mikrophone-firmware-memory-that-forgets-exploration.md) · [`20260814-021218_caum1-wire-frame-carry-only-committed-exploration.md`](20260814-021218_caum1-wire-frame-carry-only-committed-exploration.md) · [`20260814-021722_caum2-recorder-control-state-machine-exploration.md`](20260814-021722_caum2-recorder-control-state-machine-exploration.md) · [`../mikrophone/recorder.rye`](../mikrophone/recorder.rye) · [`../mikrophone/wire.rye`](../mikrophone/wire.rye) · [`../mikrophone/session.rye`](../mikrophone/session.rye)

---

## Why this rung opens

Three rungs stand, each proven alone: CAUM0's session that forgets, CAUM1's wire frame that carries only what was committed, CAUM2's record control that lets a sample land only while recording. What no rung yet proves is the three of them meeting — the whole firmware gesture a keeper actually performs: press record, speak, keep it on purpose, set the device down, and let only the kept capture cross the single wire. The pieces are known correct in isolation; the composition is the crux that has not yet been witnessed.
...
4. **A tampered frame refuses by name.** A single flipped payload byte on the composed loop's own frame refuses `DigestMismatch` before the payload is yielded — CAUM1's verify-before-trust discipline intact across the composition.
...
*Three promises, kept one at a time, become one promise kept whole: nothing crosses the wire that a keeper did not press record for, mean to keep, and choose to carry. CAUM3 writes that whole gesture into a single loop.*
