...
Only when all eight hold is the value returned. The witness proves each refusal by name, a clean round-trip for all three kinds, and — the AMUS-specific proof — a frame whose digest was *fixed up* after a vowel was smuggled into the payload still refuses `VowelPresent`, so integrity alone can never smuggle a real address across.

## The four rounds of this quest (named, this one taken)

- **AMUS3** *(this round)* — `constel/wire.rye`: frame · deframe for the three handshake values, verify-before-trust with the naming law at the border.
- **AMUS4** *(named)* — a bounded two-party exchange loop over the frames: initiator frames a `Greeting`, responder deframes and frames an `Ack`, initiator deframes and seals a `Session` — the whole AMUS2 handshake carried end to end across frames, still on one bench.
- **AMUS5** *(named)* — the local channel: two in-process piers passing frames through a bounded byte queue, no socket yet — the loopback that models the Comlink seam without crossing to a real one.
- **AMUS6** *(horizon)* — the real Comlink transport. Crosses the Comlink seam to an actual local socket between fake piers; still no real address, yet a real channel — sized and named when the bench reaches it.

## Why this is the right next step

It is the most durable move on the AMUS road: a frame layout is read on its ten-thousandth day, and this one carries the constellation's safety guarantee into every byte that will ever move between piers. It is the hardest still-tractable rung — the naming-law-at-the-border proof is the real design work — and once made, AMUS4 and AMUS5 are composition over it. And it stays wholly inside the fence: local, pure, witnessed on metal, no gate reached.
