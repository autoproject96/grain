# AMUS4 — the handshake, carried end to end across frames
...
- **A non-member and a self-greet each refuse by name**, mid-exchange, exactly as AMUS2 proved — the frame carries the fault forward faithfully.
...
It is the rung that turns three proven values and a proven frame into a proven *conversation* — the smallest whole that a real transport (AMUS5's byte queue, AMUS6's socket) will simply carry unchanged. It is durable: the exchange shape is the contract every later transport implements. It is the hardest still-tractable step on the road — the pier-boundary discipline (neither pier holds the other's value) is the real design work — and once made, AMUS5 is that same exchange with a byte queue standing in for the direct hand-off. And it stays inside the fence: local, pure, witnessed on metal, no gate reached.
