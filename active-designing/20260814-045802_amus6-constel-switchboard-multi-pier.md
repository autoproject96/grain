# AMUS6 — a switchboard: a whole local constellation, routed by name
...
**Season:** the Six-Season double-seat, Season D/F thread · **Waymark:** AMUS · rung **AMUS6**
**Kin:** [`../constel/channel.rye`](../constel/channel.rye) · [`../constel/exchange.rye`](../constel/exchange.rye) · [`../constel/roster.rye`](../constel/roster.rye) · [`../constel/README.md`](../constel/README.md) · [`20260814-044915_amus5-constel-channel-byte-stream.md`](20260814-044915_amus5-constel-channel-byte-stream.md)

---

## The crux this round takes

Every rung so far has connected exactly **two** piers — one initiator, one responder, one pair of channels. Yet *Constel* is named for a **constellation**: many piers, standing up together on one bench. AMUS1's roster already holds up to eight distinct fake piers; nothing yet lets three or more of them **meet through one shared medium**. AMUS6 takes the one new concern that a real network of many piers forces and nothing before it did: **routing a frame to the right pier by name** — every pier holding its own mailbox, a delivery reaching exactly one of them, and a stranger's name refused before a byte moves.
...
A shared medium that knows whose mailbox is whose. It leans on AMUS1's roster for membership and AMUS5's channel for each mailbox — no new value model, no new bound of its own beyond `roster.max_piers` mailboxes.
...
Two members of a constellation of `n` complete the AMUS4 handshake with the switchboard as their only carrier:
...
Every crossing is `wire.Frame` bytes through a mailbox; the switchboard only routes, so the two AMUS3 trust gates — Sha256 integrity and the naming-law border at `wire.deframe_*` — stay the **only** trust boundary, exactly where AMUS3 put them.
