...
forgets (CAUM0), a wire that carries only what was committed (CAUM1), a record
button (CAUM2), a firmware loop (CAUM3), an inbox that keeps only the proven
(CAUM4), a carry that meets device to desk (CAUM5), and a desk archive that keeps a
bounded log of proven recordings in arrival order (CAUM6).
...
move for CAUM7 is to let the desk **address recordings by that content digest**:
...
CAUM6's `archive.rye` is committed and its selftest deposits the *same* frame
...
violate accrete-never-break. So CAUM7 grows a **sibling**: `mikrophone/catalog.rye`,
