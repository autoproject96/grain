...
The whole local content-addressed sync (CAUM7–9) proved the *desk* holds and serves only what it was handed. Networked serve waits at the Comlink-served custody gate (Season 1 J2, Keaton's hand). Redaction is the next Lindy-first crux that stays purely on the device, purely in Rye — and it is the most durable expression of the module's own reason to exist, so every later surface (the field-recorder trim, the civic-microphone strike) inherits a forgetting already proven correct.
...
*A device that forgets on purpose owes its keeper a proof that a struck span is truly gone. CAUM10 writes that proof, so every trim a keeper ever makes inherits a forgetting as complete as power-down.*
