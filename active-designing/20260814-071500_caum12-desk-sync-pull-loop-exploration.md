...
- **Only the wanted crosses.** `served == wanted` — no byte crosses that the local desk did not lack, exactly the have/want economy CAUM8 named.
...
Witness `tools/caum_sync_witness.rish`; the aggregate `tools/caum_witness.rish` grows to run CAUM12 in rung order.
