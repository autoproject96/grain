# AMUS26 — Constel admit: the whole add-a-member lifecycle, closed

**Stamp:** `20260814.074656` · **Language:** EN · **Voice:** Kyri · **Style:** Radiant
**Season:** Six-Season double-seat, Season D thread (Kresfa & Mycelium — the tree's own consensus) · **waymark** AMUS · **rung** AMUS26
**Kin:** [`../constel/learner.rye`](../constel/learner.rye) (AMUS25) · [`../constel/reconfig.rye`](../constel/reconfig.rye) (AMUS13) · [`../constel/quorum.rye`](../constel/quorum.rye) (AMUS9) · [`../constel/roster.rye`](../constel/roster.rye) (AMUS1)

---

## The rung

AMUS25 gave the sky a non-voting **learner** and a `may_promote` predicate — yet nothing consumed it: a learner could catch up, but no rung actually admitted it to the vote. AMUS13 gave a **joint-consensus** membership switch — yet it would admit *any* new slot, empty log and all, the very stall AMUS25 warned against. This rung joins the two, closing the add-a-member lifecycle end to end.
...
- **AMUS15 (catchup)** carries a lagging log forward.
- **AMUS25 (learner)** is the role a member lives in while it catches up — present, replicating, counting toward nothing — and the gate `require_caught_up`.
- **AMUS13 (reconfig)** is the no-fork joint switch.
- **AMUS26 (admit)** is the join: catch-up gate, then joint switch, so the whole lifecycle is one proven arc.
...
- **The old config's majority alone cannot smuggle the switch** → a ballot of just the two old voters carries an old majority (2 of 3) yet not a new majority (2 of 4), so `require_joint` refuses `NoJointMajority`: the pre-promotion set cannot force a new member in over the new config's own head. The AMUS9 intersection lemma carries across the switch unchanged.
