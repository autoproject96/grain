# AMUS29 — Constel `lease`: a lease read, safe under a bounded clock

**Stamp:** `20260814.081032` · **Language:** EN · **Voice:** Kyri · **Style:** Radiant
**Status:** Design capture for a self-approved round · waymark **AMUS** · rung **AMUS29**
**Kin:** [`../constel/read.rye`](../constel/read.rye) (AMUS20 — the read-index) · [`../constel/tenure.rye`](../constel/tenure.rye) (AMUS24 — CheckQuorum backing) · [`../constel/README.md`](../constel/README.md)
...
- **`serve_on_lease(lease, now, lease_span, election_timeout) → bool`** — true only when the lease is sound *and* unexpired (`now − granted < lease_span`). When false, the caller falls back to the AMUS20 read-index; a lease read never replaces the safe path, it only shortcuts it.
