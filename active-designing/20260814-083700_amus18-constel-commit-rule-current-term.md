# AMUS18 — Constel commit rule: a leader commits an old-term entry only through a current-term one

**Stamp:** `20260814.083700` · **Language:** EN · **Voice:** Kyri · **Style:** Radiant
**Status:** Self-approved design round (the itinerary filling law)
**Season:** the Six-Season double-seat, Season D thread (Kresfa & Mycelium — the tree's own consensus) · **Waymark:** AMUS · **Rung:** AMUS18
**Kin:** [`../constel/README.md`](../constel/README.md) · [`../constel/repair.rye`](../constel/repair.rye) (AMUS17) · [`../constel/term.rye`](../constel/term.rye) (AMUS16) · [`../constel/quorum.rye`](../constel/quorum.rye) (AMUS9) · [`../.claude/rules/lindy-first-crux.md`](../.claude/rules/lindy-first-crux.md)

---

## The one concern this rung takes

AMUS11 committed a value when a strict majority accepted it. AMUS12 ordered committed values into a log. AMUS16 made the *leader* safe across re-election, AMUS17 made the *follower's log* safe across a rejoin. One subtle safety rule of replicated consensus is still unstated, and it is the single most famous bug-class in the whole field — Raft's §5.4.2, the scenario its paper draws as **Figure 8**:
...
The intuition that "replicated on a majority = committed" is *false for an old-term entry*. A later leader, elected on a log that happened to lack that entry, can still legally overwrite it (AMUS17's repair is exactly the mechanism that would). So counting replicas is not enough. The safe rule Raft proves is narrow and exact:

> **A leader advances its commit index over an entry only once it has replicated an entry *from its own current term* on a majority.** Committing a current-term entry commits every earlier entry beneath it *transitively*, by the Log Matching Property (AMUS17) — those older entries are now pinned on a majority whose logs match the leader's, and Leader Completeness guarantees no future leader can be elected without them.
...
AMUS11's decree commits *within a single term* — every accept in one round carries that round's term, so the committed value is always a current-term entry, and the rule holds trivially. The danger appears only once a log spans *multiple terms* and a leader reasons about entries earlier leaders wrote — exactly the world AMUS16 (terms) and AMUS17 (multi-term logs, repair) opened. AMUS18 is the safety rule that world requires.

## Shape

`constel/commit.rye` (AMUS18), purely local, standing on AMUS17 (the `TermedLog` and its Log Matching Property) · AMUS9 (`majority_of`, the intersection lemma):
...
`tools/amus_commit_witness.rish`:

1. `MatchView` counts replicas correctly (the leader always counts itself; a follower short of `n` is not counted at `n`).
2. **A single-term log** commits by majority exactly as AMUS11 did — the rule is a strict generalization, not a regression.
...
5. **The danger made real** — the old-term entry the naive rule would have committed is then *overwritten* by AMUS17's `repair` under a new leader whose log lacked it: proof that committing it naively would have forked the ledger, and that the safe rule never did.
