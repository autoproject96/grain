# AMUS19 — the committed log drives a state machine, each request applied exactly once

**Stamp:** `20260814.083900` · **Language:** EN · **Voice:** Kyri · **Style:** Radiant
**Season:** Six-Season double-seat, Season D (Kresfa & Mycelium — the tree's own consensus) · **Waymark:** AMUS · **Rung:** AMUS19
**Stands on:** [`constel/commit.rye`](../constel/commit.rye) (AMUS18 — `safe_commit_len`, the committed prefix) · [`constel/repair.rye`](../constel/repair.rye) (AMUS17 — the ordered log the commit rule draws from)
...
The Constel consensus track has climbed the whole Raft safety spine: a roster (AMUS1), a wire and a switchboard (AMUS3–6), gossip (AMUS7), a counted quorum (AMUS8–9), a leader (AMUS10), a single committed value (AMUS11), an ordered log (AMUS12), reconfiguration (AMUS13), compaction (AMUS14), catch-up (AMUS15), terms guarding a stale leader (AMUS16), log repair and the Log Matching Property (AMUS17), and the Figure 8 commit rule (AMUS18). The sky now agrees on **an ordered, safely-committed log** — and that is exactly as far as agreement goes.
...
AMUS19 introduces `constel/apply.rye`: the layer that turns AMUS18's committed prefix into applied state, with **linearizable client sessions** — each client request applied **exactly once**, its answer stable across every retry.
...
- **`apply_through(machine, cmds, commit_len)`** — fold **only** the committed prefix (`commit_len` from AMUS18's `safe_commit_len`) into the machine, in log order. An uncommitted tail never touches the state.

## What the witness proves

`tools/amus_apply_witness.rish`, six blocks:
...
3. **THE §8 REFUSAL — exactly-once under a duplicated request.** A committed log that contains one client's command **twice** (the leader-crash-then-resend that AMUS18's own danger case produced) applies it **once**: the register reflects a single application, and the second occurrence returns the cached result rather than firing again. A naive term-blind applier would double it.
...
The commit rule (AMUS18) is the last rung that made the *log* safe. Exactly-once apply is the first rung that makes the log **useful** — it is what a keeper actually experiences (my request ran once, and I got my answer). It is the highest-Lindy still-tractable move on the whole consensus road: every ledger, every mint, every transfer the Mycelium season already built (Pledge, Voucher, Portage) silently assumes a request runs once; AMUS19 is where that assumption is finally *proven* rather than hoped. It composes AMUS18's public API only, invents no new consensus, and stays entirely on the bench.
