# AMUS20 — a linearizable read without a log append (the read-index)

**Stamp:** `20260814.084100` · **Language:** EN · **Voice:** Kyri · **Style:** Radiant
**Season:** Six-Season double-seat, Season D (Kresfa & Mycelium — the tree's own consensus) · **Waymark:** AMUS · **Rung:** AMUS20
**Stands on:** [`constel/apply.rye`](../constel/apply.rye) (AMUS19 — the state machine a read observes) · [`constel/commit.rye`](../constel/commit.rye) (AMUS18 — the commit index a read captures) · [`constel/term.rye`](../constel/term.rye) (AMUS16 — the term a leader confirms)
...
## The seam AMUS19 left

AMUS19 gave the ledger a **write** that runs exactly once — a client's request applied a single time however many leaders it crossed. Yet a service is read far more than it is written, and a read has its own correctness trap, the other half of Raft's **§8**:
...
1. **Confirm leadership.** Before answering, the leader confirms a **majority still holds its term** — no newer leader has superseded it. This spends AMUS16's exact lemma: once a majority has advanced past term `T`, a leader in `T` is out-voted by construction, so a majority *still at* `T` proves the leader current.
2. **Capture the read-index.** The leader records the current **commit index** (AMUS18's `safe_commit_len`) as the read's floor, applies its state machine **through** that index (AMUS19's `apply_through`), and only then answers. The value it returns therefore reflects **every write committed before the read began**.

## The crux — a read reflects all prior writes, and a stale leader serves none

AMUS20 introduces `constel/read.rye`: a linearizable read that appends nothing.

- **`leadership_confirmed(terms, current_term, threshold)`** — `terms.at_least(current_term) >= threshold`: a majority still stands at the leader's term. The whole liveness-vs-safety hinge in one line, borrowed from AMUS16.
...
`tools/amus_read_witness.rish`, six blocks:
...
2. **The read-index is the commit index (composes AMUS18)** — `read_index` is derived by `commit.safe_commit_len` over a real `TermedLog` + `MatchView`, then fed to the read: an old-term entry not yet safely committed is **not** reflected in the value served, exactly as the write path refuses to commit it.
...
AMUS19 proved the write half of a linearizable service; AMUS20 proves the read half. Together they are a **complete linearizable key-value service** on the bench — the thing every ledger the Mycelium season built (Pledge, Voucher, Portage) is, underneath. It is the highest-Lindy still-tractable move: a correct read is what a keeper experiences on every single interaction that is not a write, which is most of them. It composes AMUS16 · AMUS18 · AMUS19 public API only and invents no new consensus.
