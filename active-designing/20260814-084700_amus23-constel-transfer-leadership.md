# AMUS23 — Constel leadership transfer: a handoff only to a caught-up successor

**Stamp:** `20260814.084700` · **Language:** EN · **Voice:** Kyri · **Style:** Radiant
**Season:** Six-Season double-seat, Season D thread (Kresfa & Mycelium — the tree's own consensus) · **waymark** AMUS · **rung** AMUS23
**Kin:** [`../constel/vote.rye`](../constel/vote.rye) (AMUS21) · [`../constel/prevote.rye`](../constel/prevote.rye) (AMUS22) · [`../constel/repair.rye`](../constel/repair.rye) (AMUS17) · [`../constel/quorum.rye`](../constel/quorum.rye) (AMUS9)
...
2. It brings the chosen target **fully up to date** — AMUS17's catch-up / repair — until the target's log head matches its own.
...
A transfer is authorized **only to a target that is caught up** — same last term, same length as the leader. So the successor holds every committed entry, and **Leader Completeness (AMUS21) is preserved, not bypassed.** A behind target is refused by name (`TargetNotCaughtUp`); TimeoutNow is never sent to a log that lacks the sky's settled history. An out-of-roster target is refused too (`TargetOutOfRoster`).

Because a follower's log can never be strictly more up-to-date than the leader that feeds it, "at least as current as the leader" (AMUS21's measure, applied with the leader as the bar) collapses to an **exact head match**. That is why `is_caught_up` is a clean equality of `(last_term, len)`, guarded by the invariant that the target is never strictly ahead of its source.

## The one legitimate skip of pre-vote

AMUS22's pre-vote protects a stable sky from a *partitioned node's* disruption. A leader-orchestrated transfer is the opposite motion: the current leader has personally verified the successor and is stepping aside for it. So the practice round is unnecessary — the transfer's `TimeoutNow` deliberately skips it, and the campaign term is exactly `leader_term + 1`, a single advance.
