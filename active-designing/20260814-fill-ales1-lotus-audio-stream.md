...
1. **Delimiting** — cut exactly one whole frame out of the stream, waiting (`NeedMoreBytes`) without moving the cursor when a frame is not yet complete, refusing a wild declared length (`BadFrame`) before any over-read. The same discipline AMUS5 proved, on the shared LOTS header (payload length at offset 6).
