# Constel naming law — fake piers that can never be a real ship (AMUS0)
...
- **This round seats the law and the name primitive only** — validate, prove-never-a-ship, generate. The fake-pier harness (spinning up local piers, Comlink handshake between them) crosses the Comlink seam and is its own later AMUS round.
...
- Witness: `tools/amus_name_witness.rish` proves valid names accepted, every real `@p` example (`zod`, `sarlev`, `sampel`, `palnet`, `sampel-palnet`) rejected for its vowel, generation deterministic and collision-free across a bounded sweep, and every generated name provably never a ship.
