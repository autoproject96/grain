...
# so the molt (FUSS) knows exactly what to touch:
#
#   kind  — prose  : a living prose surface naming a capability by a bare "lap N".
#                    FUSS relabels these to semantic label + stamp.
...
#           gap    : no guard watches it yet — the set FUSS must grow the guard to cover.
#
